...
public class MemoryGameManager : MonoBehaviour
{
	...
	private void CreateCardsContainer()
	{
		...
		cardsContainer.AddComponent<LazyFollow>(); // Aggiungi XRRayInteractor per interazioni VR
		cardsContainer.GetComponent<LazyFollow>().target = Camera.main.transform;
		cardsContainer.GetComponent<LazyFollow>().targetOffset = new Vector3(0f, 0f, 0.3f); // Offset per la visualizzazione corretta
	...
	void InstantiateCard(GameObject cardPrefab, Vector3 position)
	{
		...
		// card.GetComponent<LazyFollow>().target = Camera.main.transform;
		// Vector3 offset = new Vector3(position.x * 0.1f, (position.y * 0.1f) - 0.5f * 0.1f);
		// card.GetComponent<LazyFollow>().targetOffset = offset;
	...
	void HideCards()
	{
		...
		selectedCard1.GetComponent<CheckHitbox>().glowEffect.SetActive(false);
		selectedCard2.GetComponent<CheckHitbox>().glowEffect.SetActive(false);
	}
	...
	void HideAllCards()
	{
		foreach (var card in spawnedCards)
		{
			if (card != null)
			{
				...
				if (memoryCard != null)
					memoryCard.Hide();
				memoryCard.GetComponent<CheckHitbox>().glowEffect.SetActive(false);
			}
		}
	}
	...
	void Update()
	{
		foreach (var card in spawnedCards)
		{
			if (card != null)
			{
				var hitbox = card.GetComponent<CheckHitbox>();
				var memoryCard = card.GetComponent<MemoryCard>();
				if (hitbox != null && hitbox.glowEffect != null && hitbox.glowEffect.activeSelf && memoryCard != null && !memoryCard.IsPermanentlyRevealed)
					CardSelected(card);
			}
		}
	}
